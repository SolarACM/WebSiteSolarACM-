...
export default function QuotePage() {
  ...
  return <PageShell lang={lang} setLang={setLang}>
    <style>{`
      ...
      @media(max-width:620px){.quote-form{padding:22px}.quote-grid,.choice-grid{grid-template-columns:1fr}.service-grid{grid-template-columns:1fr 1fr}.service-option{min-height:86px}}
    `}</style>
    <VisualHero image="/portfolio/project-03.jpg" imagePosition="center 44%" imageAlt={isTh?"โครงการโซลาร์รูฟท็อปสำหรับขอคำปรึกษา":"Solar rooftop project consultation"} kicker="Consultation request" title={isTh?"เริ่มต้นโครงการด้วยข้อมูลที่คุณมี":"Start your project with the information you have"} lead={isTh?"ไม่จำเป็นต้องรู้พื้นที่หลังคาหรือสเปกอุปกรณ์ ทีมงานจะติดต่อกลับเพื่อถามเฉพาะข้อมูลที่จำเป็นในขั้นถัดไป":"Exact roof area or equipment specifications are not required. Our team will follow up with the next relevant questions."}/>
    <section className="site-section site-section--soft"><div className="site-container quote-layout"><aside className="quote-aside"><h2>{isTh?"ก่อนส่งข้อมูล":"Before you submit"}</h2><p>{isTh?"แบบฟอร์มนี้รองรับทั้งโครงการพลังงานและการสอบถามผลิตภัณฑ์":"This form supports both energy projects and product enquiries."}</p><ul><li><Check size={17}/>{isTh?"กรอกเฉพาะข้อมูลที่ทราบ":"Provide only what you know"}</li><li><Check size={17}/>{isTh?"แนบบิลค่าไฟได้หากเป็นงานโซลาร์":"Attach a bill for solar enquiries"}</li><li><Check size={17}/>{isTh?"ข้อมูลจะใช้เพื่อการติดต่อกลับและประเมินเบื้องต้น":"Information is used for follow-up and initial assessment"}</li></ul></aside>
      <form className="quote-form" onSubmit={submit} noValidate><h2>{isTh?"ข้อมูลสำหรับติดต่อกลับ":"Contact details"}</h2><p>{isTh?"ช่องที่มีเครื่องหมาย * จำเป็นต่อการติดต่อเบื้องต้น":"Fields marked * are required for the initial follow-up."}</p>
        ...
        <Field label={isTh?"แนบรูปบิลค่าไฟ (ไม่บังคับ)":"Attach bill image (optional)"} hint={isTh?"JPG, PNG, WebP หรือ HEIC ขนาดไม่เกิน 5 MB":"JPG, PNG, WebP, or HEIC up to 5 MB"} error={errors.file}><input ref={fileRef} type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" hidden onChange={(e)=>chooseFile(e.target.files?.[0])}/><div className="upload-box" role="button" tabIndex={0} onClick={()=>fileRef.current?.click()} onKeyDown={(e)=>e.key==="Enter"&&fileRef.current?.click()}>{preview?<Image src={preview} alt="Bill preview" width={64} height={64} unoptimized/>:<Upload size={26}/>}<div><strong>{file?file.name:(isTh?"เลือกไฟล์รูปภาพ":"Choose image")}</strong><span>{file?`${(file.size/1024/1024).toFixed(2)} MB`:(isTh?"คลิกเพื่อเลือกไฟล์":"Click to select a file")}</span></div>{file&&<button className="upload-remove" type="button" aria-label="Remove file" onClick={(e)=>{e.stopPropagation();setFile(null);setPreview("");}}><Trash2 size={19}/></button>}</div></Field>
