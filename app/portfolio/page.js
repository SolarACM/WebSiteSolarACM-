...
export default function PortfolioPage() {
  ...
  return (
    <PageShell lang={lang} setLang={setLang}>
      <style>{`
        .portfolio-hero{padding:96px 0 80px;background:var(--green-950);color:white}.portfolio-hero__inner{max-width:900px}.portfolio-hero h1{margin:0;font-size:clamp(2.7rem,5vw,4.8rem);line-height:1.08;letter-spacing:0}.portfolio-hero p{max-width:760px;margin:24px 0 0;color:rgba(255,255,255,.68);font-size:1.08rem}
        .portfolio-note{display:flex;gap:12px;margin-top:30px;padding:18px;border:1px solid rgba(255,255,255,.16);color:rgba(255,255,255,.72);font-size:14px}.portfolio-note svg{flex:0 0 auto;color:#8cd4a7}
        ...
        @media(max-width:760px){.portfolio-hero{padding:68px 0}.portfolio-grid{grid-template-columns:1fr}.portfolio-card__image{height:260px}.portfolio-modal{padding:10px}.portfolio-modal__body{padding:20px}}
      `}</style>
      <section className="portfolio-hero"><div className="site-container portfolio-hero__inner"><span className="site-kicker">Portfolio</span><h1>{isTh?"ผลงานและข้อมูลโครงการจากเครือข่าย":"Project information from our network"}</h1><p>{isTh?"เราแสดงเฉพาะข้อมูลที่มีอยู่ในชุดข้อมูลโครงการ โดยไม่เปิดเผยชื่อลูกค้า และไม่ทำให้เข้าใจว่า Solar ACM เป็น EPC ของทุกงาน":"We publish only information available in the project records, protect client identities, and do not imply Solar ACM executed every project as EPC."}</p><div className="portfolio-note"><Building2 size={20}/><span>{isTh?"ชื่อ EPC ผู้ดำเนินการและบทบาทของ Solar ACM ในแต่ละโครงการยังอยู่ระหว่างการยืนยันจากเจ้าของข้อมูล จึงยังไม่เผยแพร่บนเว็บไซต์":"Executing EPC names and Solar ACM's project-specific role are pending confirmation from the data owner and are not yet published."}</span></div></div></section>
      <section className="site-section site-section--soft"><div className="site-container"><div className="portfolio-grid">{projects.map((project)=><button className="portfolio-card" key={project.id} onClick={()=>setSelected(project)}><div className="portfolio-card__image"><img src={project.image} alt={project.title} loading="lazy"/><span className="portfolio-card__tag">Industrial rooftop</span></div><div className="portfolio-card__body"><h2>{project.title}</h2><div className="portfolio-meta"><span><Zap size={15}/>{project.capacity} kWp</span><span><MapPin size={15}/>{project.province}</span><span><Calendar size={15}/>{project.year}</span></div></div></button>)}</div></div></section>
